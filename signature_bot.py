...
PORT = 8080 # Koyeb uses port 8080 by default
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    signed_text = f"{update.message.text}{SIGNATURE_TEXT}"
...
async def handle_media_and_others(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if update.message.photo:
            await context.bot.send_photo(chat_id=TARGET_CHANNEL_ID, photo=update.message.photo[-1].file_id, caption=signed_caption)
            reply_text = "Photo forwarded!"
        elif update.message.video:
            await context.bot.send_video(chat_id=TARGET_CHANNEL_ID, video=update.message.video.file_id, caption=signed_caption)
            reply_text = "Video forwarded!"
        elif update.message.audio:
            await context.bot.send_audio(chat_id=TARGET_CHANNEL_ID, audio=update.message.audio.file_id, caption=signed_caption)
            reply_text = "Audio forwarded!"
        elif update.message.document:
            await context.bot.send_document(chat_id=TARGET_CHANNEL_ID, document=update.message.document.file_id, caption=signed_caption)
            reply_text = "Document forwarded!"
            ...
            reply_text = "Sticker forwarded!"
        else:
            await update.message.forward(chat_id=TARGET_CHANNEL_ID)
            await update.message.reply_text("Forwarded without signature.")
            return
...
async def main_bot():
    if not BOT_TOKEN or not TARGET_CHANNEL_ID:
        print("FATAL: BOT_TOKEN or TARGET_CHANNEL_ID not found in environment variables!")
        return
